...
def insects(state):
    if not state["initialised"]:
        ...
        state["insects"]["scene"] = engine.Scene(size=(800, 600),
                                                 sprite_named_filenames={"default": "images/insects_bg.jpg"})
        state["insects"]["entities"]["player"] = engine.GameEntity((350, 500), (51, 146),
                                                                   {"default": "images/gun2.png"})
    ...
    if state["insects"]["flycount"] < 8:
        if state["insects"]["flycooldown"] == 0:
            state["insects"]["flycount"] += 1
            state["insects"]["flycooldown"] = 70
            fly = engine.Trigger((random.randint(0, 5) * 160, -80), (64, 64), {"default": "images/bug.png"},
                                 func=fly_fate, type={"collides"})
        ...
        if entity_name[:3] == "fly":
            ...
            for entity_name_in in state["insects"]["entities"]:
                if entity_name_in[:3] == "clo":
                    state = fly.check(state, state["insects"]["entities"][entity_name_in],
                                      params=[entity_name, entity_name_in])
