...
def insects(state):
    if not state["initialised"]:
        ...
        state["village"]["music"] = pygame.mixer.music.load("sounds/insects.mp3")
    ...
    if state["insects"]["lives"] <= 0 or state["insects"]["score"] <= 0:
        ...
        if state["insects"]["lives"] <= 0:
            state["insects"]["end_text"].text = "Мухи одолели Вас.\nОсталось попыток: "\
            + str(state["progress"][state["current_task"]]["attempts"] - 1)


        else:
            state["insects"]["end_text"].text = "Вы победили!\nПриходите к жителю за запиской."
            state["progress"][state["current_task"]] = {"available": False, "attempts": 9999, "completed": True}
